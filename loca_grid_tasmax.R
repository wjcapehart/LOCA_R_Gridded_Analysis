...
variable_human_name = "Max Daily Temperature"

variable_prefered_units = "deg C"

yearly_aggregation_label = "Mean"
...
################################################################
  ...
  for (ens in 1:n_ensembles) {
      ...
      sumprec_xye[,,ens]    = apply(arr_xyt, c(1,2), mean)
...
################################################################
    ...
    for (ens in 1:n_ensembles) {
      ...
      time_period = time[t_start:t_end]
      ...
      for (i in 1:nx) {
        ...
        for (j in 1:ny)  {
          ...
          threshold_value = quantile(x     = time_series[j,],
                                     probs = 0.80,
                                     names = FALSE
                                     )
      ...
      print(paste(i,j,ensemble_site[ens],threshold_value,return_xyer[1,1,ens,9]))
      plot(x    = time_period,
           y    = time_series[ny,],
           type = "l",
           main = paste("LLC ",
      ...
      filled.contour(x         = longitude,
                     ...
                     main      = paste(start_year[period],
                                       ...
                                       " ",
                                       ensemble_site[ens],
                                       " Mean Annual Daily Max Temp (degC)",
                                       sep = ""),
                     asp       = 1,
                     color = matlab.like,
                     plot.axes = { map(database = "county",
  ...
  # mean_xye
  netcdf_mean_xye = ncvar_def(nam      = "mean_period_daily_max_temp",
                              units    = "degC",
                              dim      = list(netcdf_lon_dim,
                                              netcdf_lat_dim,
                                              netcdf_ens_dim),
                              missval  = fill_value,
                              longname = "Mean Daily Max Temps for Period",
                              prec     = "single")

  #stdv_xye
  netcdf_stdv_xye = ncvar_def(nam      = "stdev_period_daily_max_temp",
                              units    = "degC",
                              dim      = list(netcdf_lon_dim,
                                              netcdf_lat_dim,
                                              netcdf_ens_dim),
                              missval  = fill_value,
                              longname = "Std Dev Daily Max Temps for Period",
                              prec     = "single")


  # mean_xyde
  netcdf_mean_xyde = ncvar_def(nam      = "annual_daily_max_temp",
                              units    = "degC",
                              dim      = list(netcdf_lon_dim,
                                              netcdf_lat_dim,
                                              netcdf_year_dim,
                                              netcdf_ens_dim),
                              missval  = fill_value,
                              longname = "Mean Annual Daily Max Temps",
                              prec     = "single")

  #   return_xyer = arr_xyer
  netcdf_return_xyer = ncvar_def(nam      = "daily_max_temp_return",
                                 units    = "degC",
                                 dim      = list(netcdf_lon_dim,
                                                 netcdf_lat_dim,
                                                 netcdf_ens_dim,
                                                 netcdf_retper_dim),
                                 missval  = fill_value,
                                 longname = "Daily Max Temperature Returns",
                                 prec     = "single")
