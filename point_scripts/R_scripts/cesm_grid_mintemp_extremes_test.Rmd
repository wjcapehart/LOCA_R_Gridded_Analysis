...
library(ncdf4.helpers) # support for working with netCDF data
...
location_name = "Rapid City"
   target_lon = -103.2310 # degrees east
   target_lat =   44.0805 # degrees north
...
variable_name   = "Daily Min Temperatures"
variable_units  = "°C" # after post processing (starts as m/s)

threshold_value = (0.-32) * 5/9.
...
var2d.45 = ncvar_get(nc      = nc.45,                      # netcdf file ID
                     ...
                     ) -273.15                  # scaling temperature from K to DegF
...
var2d.85 = ncvar_get(nc      = nc.85,                      # netcdf file ID
                     ...
                     start   = c( 1,  1,  1,  1),          # starting coordinates
                     count   = c(10,  7, n_ensembles, nt)  # end coordinates (we're only going to 2080 and using 15 ensembles)
                     ) -273.15              # scaling temps from K to degF
...
fit_GP  = fevd(x          = RCP45, 
               ...
               units      = paste("[Neg]",variable_units),
               time.units = "365/year",
               type       = "GP",
               span       = period_span
               )
...
for (ens_target in 1:n_ensembles) {
  ...
  rcp45_base_period_ci_lb[ ens_target, ] = -return_GP[ , 3]
  rcp45_base_period_return[ens_target, ] = -return_GP[ , 2]
  rcp45_base_period_ci_ub[ ens_target, ] = -return_GP[ , 1]
  ...
  rcp85_base_period_ci_lb[ ens_target, ] = -return_GP[ , 3]
  rcp85_base_period_return[ens_target, ] = -return_GP[ , 2]
  rcp85_base_period_ci_ub[ ens_target, ] = -return_GP[ , 1]
  ...
  rcp45_per1_period_ci_lb[ ens_target, ] = -return_GP[ , 3]
  rcp45_per1_period_return[ens_target, ] = -return_GP[ , 2]
  rcp45_per1_period_ci_ub[ ens_target, ] = -return_GP[ , 1]
  ...
  rcp85_per1_period_ci_lb[ ens_target, ] = -return_GP[ , 3]
  rcp85_per1_period_return[ens_target, ] = -return_GP[ , 2]
  rcp85_per1_period_ci_ub[ ens_target, ] = -return_GP[ , 1]
...
beanplot(formula     = RCP85_Return_Period~return_intervals,  # formula selection for y axis
         ...
         main        = paste(location_name, 
                             " CESM Return Intervals for ",         # title caption
                             base_start,
                            "-",
                            base_end,
                            sep=""),
         xlab        = "Return Period (Years)",               # xaxis title
         ylab        = paste("Minimum ",variable_name,                   # yaxis title
                             " (",
                             variable_units,
                             ")",
                             sep=""),
...
deleteme     = data.frame(Ensembles        = rcp45_per1$Ensembles,
                          return_intervals = rcp45_per1$return_intervals,
...
beanplot(formula     = Return_Period_Values~merged_period_scenarios,  # formula selection for y axis
         ...
         side        = "both",
         main        = paste(location_name,
                             "CESM Return Intervals for RCP 8.5",
                             sep = " "),         # title caption
         xlab        = "Return Period (Years)",               # xaxis title
         ylab        = paste("Minimum ",
                             variable_name,                   # yaxis title
...
beanplot(formula     = Return_Period_Values~merged_period_scenarios,  # formula selection for y axis
         ...
         side        = "both",
         main        = paste(location_name,
                             "CESM Return Intervals for RCP 4.5",
                             sep = " "),         # title caption
         xlab        = "Return Period (Years)",               # xaxis title
         ylab        = paste("Minimum ",
                             variable_name,                   # yaxis title
